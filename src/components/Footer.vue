<template>
  <div class="bg-[#121A1D] w-screen px-4 sm:px-8 lg:px-30 py-6 sm:py-8 text-[#797B78]">
    <div v-if="!copyrightOnly" class="flex w-full justify-between gap-8 sm:gap-16 lg:gap-40 flex-col sm:flex-row mb-6">
      <div class="w-full sm:w-150 flex flex-col gap-2">
        <div>
          <img src="@/assets/logo.png" alt="" class="w-40 sm:w-45 lg:w-55" />
        </div>
        <div class="text-sm sm:text-base">
          Cebu Crust brings authentic Italian pizza to Cebu City. We use traditional wood-fired ovens and premium ingredients. Located near University of Cebu Main Campus, we're your go-to destination for delicious and fresh pizza.
        </div>
      </div>
    ...
    <!-- Separator -->
    <div v-if="!copyrightOnly" class="border-t border-dashed border-[#797B78] mb-4"></div>
    ...
    <div class="flex justify-between flex-col sm:flex-row gap-4 sm:gap-0">
      ...
      <div class="flex gap-4 sm:gap-8">
        <button class="text-sm sm:text-base hover:text-white transition-colors">Privacy Policy</button>
        <button class="text-sm sm:text-base hover:text-white transition-colors">Terms of Use</button>
      </div>
    </div>
  </div>
</template>
