...
const modifyOrder = async () => {
  ...
  await order.updateOrder(Number(route.params.id), currentOrder)
}
...
const removeCount = (pizzaId: number) => {
  order.setPendingOrder(
    order.pendingOrder.map((c) =>
      pizzaId === c.pizzaId ? { pizzaId: c.pizzaId, quantity: c.quantity - 1 } : c,
    ),
  ...
  <div class="min-h-screen bg-gray-50">
    ...
    <div class="py-8 min-h-[calc(100vh-5rem)]">
      ...
      <div class="w-screen px-4 sm:px-8 lg:px-30 mb-8">
        <h1 class="text-3xl font-bold text-gray-900 mb-2">Modify Your Order</h1>
        <p class="text-gray-600 text-lg">Review your items or complete your delivery details</p>
      </div>
      ...
      <div class="w-screen px-4 sm:px-8 lg:px-30 grid grid-cols-1 lg:grid-cols-5 gap-6">
                    ...
                    <div class="flex items-center justify-between">
                      ...
                      <div class="flex items-center gap-2">
                        ...
                        <span class="px-3 py-1 bg-gray-700 rounded text-white font-medium">{{
                          pizzaOrder.find((c) => c?.pizzaId === item?.pizzaId)?.quantity || 1
                        }}</span>
                        <button
                      ...
                      <div class="flex items-center gap-2">
                        ...
                        <Button
                          variant="ghost"
                          :disabled="order.pendingOrder.length <= 1"
                          @click="
                            order.setPendingOrder(
                              order.pendingOrder.filter((o) => o.pizzaId !== item?.pizzaId),
                            )
                          "
                          class="hover:bg-gray-700 rounded-full transition-colors"
                        >
        ...
        <div class="lg:col-span-3">
          ...
          <div class="flex gap-4">
            <Button @click="modifyOrder" class="h-12 px-6 py-3 rounded-lg font-medium">
              Modify Order
            </Button>
            <Button
              variant="outline"
