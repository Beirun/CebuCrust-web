...
const displayName = computed(() => auth.user?.name || auth.user?.userName || auth.user?.username || '')

// Addresses stored locally; keep simple shape { id, address, isDefault }
const addresses = ref<Array<{ id: string; address: string; isDefault?: boolean }>>(JSON.parse(localStorage.getItem('addresses') || '[]'))
const selectedAddressId = ref<string | null>(addresses.value.find(a => a.isDefault)?.id ?? (addresses.value[0]?.id ?? null))
...
const submitAddress = () => {
  ...
  if (editing.value && editing.value.id) {
    addresses.value = addresses.value.map(a => a.id === editing.value!.id ? { ...a, address: trimmed } : a)
  } else {
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-16">
          ...
          <nav class="hidden md:flex space-x-8">
            ...
            <router-link to="/orders" class="text-orange-500 font-medium">Orders</router-link>
    ...
    <div class="py-8">
      ...
      <div class="lg:col-span-1">
        <div class="bg-gray-900 text-white rounded-lg p-6 shadow">
          ...
          <p class="text-gray-300 text-sm mb-6">Review your items and complete your delivery details</p>

          <div class="bg-gray-800 rounded-lg p-4">
            <h3 class="text-white font-semibold mb-4">Your Order</h3>
            <div v-for="item in cart.items" :key="item.id" class="flex items-center justify-between bg-gray-700 p-3 rounded mb-3">
              <div class="flex items-center gap-3">
                <img v-if="item.image" :src="item.image" class="w-12 h-12 rounded-full object-cover" />
                <div>
                  <div class="font-medium">{{ item.name }}</div>
                  <div class="text-xs text-gray-200">{{ item.description || '' }}</div>
                  <div class="flex items-center gap-2 mt-2">
                    <button @click="cart.updateQuantity(item.id, (item.quantity || 1) - 1)" class="bg-orange-500 text-white rounded-full w-6 h-6">-</button>
              ...
              <div class="text-right">
                <div class="font-semibold">{{ formatCurrency(item.price * (item.quantity || 1)) }}</div>
                <button @click="cart.removeItem(item.id)" class="text-red-400 text-sm mt-2">🗑️</button>
              </div>
            </div>

            <div class="mt-4 border-t border-gray-600 pt-4 text-sm">
              <div class="flex justify-between text-gray-300 mb-2"><span>Subtotal</span><span>{{ formatCurrency(subtotal) }}</span></div>
              <div class="flex justify-between text-gray-300 mb-2"><span>Delivery Fee</span><span>{{ formatCurrency(deliveryFee) }}</span></div>
              <div class="flex justify-between font-semibold text-orange-300 text-lg"><span>Total Amount</span><span>{{ formatCurrency(total) }}</span></div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="lg:col-span-2">
        <div class="bg-white rounded-lg p-6 shadow mb-6 border">
          ...
          <div v-if="addresses.length" class="space-y-3">
            <div v-for="a in addresses" :key="a.id" class="flex items-center justify-between p-3 border rounded">
              ...
              <div class="flex items-center gap-2">
                ...
                <button v-if="!a.isDefault" @click="setDefault(a.id)" class="text-sm text-orange-500">Set as Default</button>
              </div>
            </div>
          </div>
          ...
          <div class="mt-4">
            <button @click="startAdd" class="bg-orange-500 text-white px-4 py-2 rounded">+ Add New Address</button>
          </div>
        </div>

        <div class="bg-white rounded-lg p-6 shadow mb-6 border">
          <h3 class="font-semibold mb-4">Special Instructions</h3>
          <textarea v-model="instructions" rows="4" class="w-full border rounded p-3 text-sm" placeholder="Ring doorbell twice. Leave at front door if no answer."></textarea>
        </div>

        <div class="flex gap-4">
          <button @click="placeOrder" class="bg-orange-500 text-white px-6 py-3 rounded shadow">Place Order</button>
          <router-link to="/menu" class="border border-orange-300 text-orange-500 px-6 py-3 rounded">Back to Menu</router-link>
        </div>
      </div>
    </div>

    <!-- Add/Edit address modal (simple) -->
    <div v-if="openAdd" class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center">
      <div class="bg-white rounded-lg p-6 w-full max-w-md">
        <h4 class="font-semibold mb-4">{{ editing && editing.id ? 'Edit Address' : 'Add Address' }}</h4>
        <textarea v-model="addressInput" rows="4" class="w-full border rounded p-3 mb-4" placeholder="123 Lahug Street, Barangay Lahug, Cebu City, 6000"></textarea>
        <div class="flex justify-end gap-2">
          <button @click="openAdd = false" class="px-4 py-2 border rounded">Cancel</button>
          <button @click="submitAddress" class="px-4 py-2 bg-orange-500 text-white rounded">Save</button>
        </div>
      </div>
    </div>
    </div>
